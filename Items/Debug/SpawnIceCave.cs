...
public class SpawnIceCave : ModItem {
    ...
    public override bool? UseItem(Player player) {
        ...
        bool facingLeft = WorldGen.genRand.Next(0, 2) == 0;
        ...
        //hanging icicles
        int iterations = WorldGen.genRand.Next(w / 20 + 2, w / 10 + 3);
        for (byte i = 0; i < iterations; i++) {
            double tailXOffset = w * 1.5 / iterations * i + WorldGen.genRand.Next(-3, 3) - w / 2;
            double pointXOffset = WorldGen.genRand.Next(-(w / 15) - 1, w / 15 + 2);

            WorldUtils.Gen(new Point(x + (int)tailXOffset, y - h - tunnelHeight), new Shapes.Tail(w / 8 + 5,
                    new Vector2D(pointXOffset, h / 3.5 + WorldGen.genRand.Next(0, (int)(h / 1.5)))),
                Actions.Chain(
                    new Actions.SetTile(TileID.IceBlock, true),
        ...
        // entry cave
        byte caveNum = (byte)WorldGen.genRand.Next(1, 3);
        if (facingLeft)
            for (byte i = 0; i < caveNum; i++)
                WorldGen.digTunnel(x - w, y - tunnelHeight / 2, -1.2, 0, 140,
