...
public static class WorldGenHelper {
    ...
    public static void GenerateMainHouse() {
        ...
        for (ushort counts = 0; counts < 10; counts++) {
            initialX = WorldGen.genRand.Next(Main.spawnTileX - 25, Main.spawnTileX + 25);
            if (!spawnUnderworld)
    ...
    public static void GenerateBeachHouse() {
        ...
        while (Main.tile[tileX, tileY].LiquidAmount == 0) tileY++;

        int direction = placeRightSide ? -1 : 1;
        do {
            // get the x when liquid ends
            while (Main.tile[tileX, tileY].LiquidAmount != 0 || Main.tile[tileX + direction * 75, tileY].LiquidAmount != 0) tileX += direction;

            // go up until just before the liquid ends
            while (Main.tile[tileX, tileY - 1].LiquidAmount != 0) tileY--;
        ...
        try {
            ...
            if (WorldGen.genRand.Next(0, 3) == 0) // 1/3 chance
            {
                ...
                if (!placeRightSide)
                    x = tileX - 9 + 35 + WorldGen.genRand.Next(8, 12);
                else
                    x = tileX - 23 - WorldGen.genRand.Next(8, 12);

                int y = tileY - 20;
                while (y < Main.worldSurface) {
                    if (WorldGen.SolidTile(x, y)) break;
                    y++;
                }
