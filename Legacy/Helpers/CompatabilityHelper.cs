...
using SpawnHouses.Legacy.Testing;
using Terraria;
...
public class CompatabilityHelper : ModSystem {
    ...
    public static void GetMagicStorage() {
        ...
        try {
            ...
            IsMSEnabled = false;
            ErrorLoadingMS = true;
            SpawnHousesMod.Instance.Logger.Error("Failed to retrieve Magic Storage IDs. Contact the mod author about this issue");
        }
    }
    ...
    public static void LinkRemoteStorage(Point16 remotePos, Point16 heartPos) {
        ...
        void SendError() {
            SpawnHousesMod.Instance.Logger.Error("Failed to link Magic Storage's remote storage to storage heart. Contact the mod author about this issue");
        }
    ...
    public static void UpdateStorageNetwork(int x, int y) {
        NetHelper.SendSearchAndRefresh(x, y);
    }
    ...
    public override void OnModLoad() {
        if (ModLoader.HasMod("MagicStorage") && SpawnHousesMod.Config.MagicStorageIntegrations)
            GetMagicStorage();
