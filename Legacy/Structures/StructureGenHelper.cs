...
public static class StructureGenHelper {
    ...
    public static void GenerateBeams(Point16 start, Tile beamTile, ushort beamInterval, ushort beamsCount, ushort maxBeamSize = 50, bool stopOnNonSolidTile = false) {
        ...
        // set the tile frames
        WorldUtils.Gen(new Point(start.X + (beamInterval + 1) * beamsCount, start.Y + maxBeamSize / 2), new Shapes.Circle(maxBeamSize), new Actions.SetFrames());
    }
    ...
    public static void GenerateFoundation(Point16 start, ushort tileID, int foundationRadius, bool useHalfCircle = false) {
        if (useHalfCircle) {
