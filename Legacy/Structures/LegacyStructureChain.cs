...
public abstract class LegacyStructureChain { // : StructureRoot {
    ...
    private readonly TileBox[] _startingBoundingBoxes;
    private List<TileBox> _boundingBoxes;
    private LegacyChainStructure[] _copiedStructureList;
    ...
    private LegacyChainStructure[] _usableStructureList;
    ...
    public LegacyChainStructure RootStructure;
    public byte Status;
    public bool SuccessfulGeneration = true;

    protected LegacyStructureChain(ushort minCost, ushort maxCost, byte minBranchLength, byte maxBranchLength,
        ushort entryPosX,
        ushort entryPosY, LegacyChainStructure[] structureList, Bridge[] bridges,
        LegacyChainStructure[] rootStructureList = null,
        TileBox[] startingBoundingBoxes = null,
        ...
        _startingBoundingBoxes = startingBoundingBoxes;
        Status = status;
    }
    ...
    public void CalculateChain() {
        foreach (LegacyChainStructure structure in _originalStructureList)
            if (structure.Cost < 0)
        ...
        _copiedStructureList = (LegacyChainStructure[])_originalStructureList.Clone();
        ...
        for (int attempts = 0; attempts < 10000; attempts++) {
            ...
            _boundingBoxes = [];
            if (_startingBoundingBoxes is not null && _startingBoundingBoxes.Length != 0)
                _boundingBoxes.AddRange(_startingBoundingBoxes);
            _boundingBoxes.AddRange(RootStructure.DetailedBoundingBoxes);

            var points = _queuedConnectPoints; // because scope
            RootStructure.ActionOnEachChainConnectPoint(connectPoint => {
        ...
        if (!foundValidStructureChain) {
            SpawnHousesMod.Instance.Logger.Error(
                $"Failed to generate LegacyStructureChain of type {ToString()}. Please report this error, this seed ({Main.ActiveWorldFileData.Seed}), and your client.log to the mod's author");
        ...
        ActionOnEachStructure(structure => {
            if (StructureIdHelper.IsBranchingHallway(structure)) {
                ...
                {
                    structure.ActionOnEachChainConnectPoint(point => { _failedConnectPointList.Remove(point); });
    ...
    private void CalculateWeights() {
        _usableStructureList = (LegacyChainStructure[])_copiedStructureList.Clone();
        ...
        for (int i = 1; i < _copiedStructureList.Length; i++)
            _usableStructureList[i].Weight = (ushort)(_usableStructureList[i - 1].Weight + _copiedStructureList[i - 1].Weight);
    }

    private LegacyChainStructure PlaceNewStructure(ChainConnectPoint parentConnectPoint, bool closeToMaxBranchLength = false, int x = 500, int y = 500) {
        LegacyChainStructure structure = null;
        for (int i = 0; i < 100; i++) {
            ...
            if (structure is not null) {
                ...
                structure.ParentLegacyStructureChain = this;
                break;
            }
        }
    ...
    private void MoveConnectPointAndStructure(LegacyChainStructure structure, ChainConnectPoint connectPoint, int x, int y) {
        Point16 delta = new(connectPoint.X - x, connectPoint.Y - y);
        Point16 newPos = structure.BoundingBox.TopLeftPoint16 - delta;
        structure.SetPosition(newPos.X, newPos.Y);
    }
    ...
    private void CalculateChildrenStructures(ChainConnectPoint connectPoint) {
        ...
        LegacyChainStructure newStructure = null;
        ...
        for (int findLocationAttempts = 0; findLocationAttempts < 15; findLocationAttempts++) {
            ...
            if (!TileBox.AnyColliding(newStructure.DetailedBoundingBoxes, _boundingBoxes) &&
                !TileBox.AnyColliding(connectPointBridge.BoundingBoxes, _boundingBoxes)) {
        ...
        _boundingBoxes.AddRange(newStructure.DetailedBoundingBoxes);
        ...
        connectPoint.ChildStructure.BridgeDirectionHistory =
            LegacyChainStructure.CloneBridgeDirectionHistory(connectPoint.ParentStructure);
    ...
    public virtual bool Generate() {
        ...
        ActionOnEachStructure(structure => {
            ...
            structure.ActionOnEachChainConnectPoint(connectPoint => {
    ...
    public void ActionOnEachStructure(Action<LegacyChainStructure> function) {
        void Recursive(LegacyChainStructure structure) {
            function(structure);
            structure.ActionOnEachChainConnectPoint(connectPoint => {
    ...
    ///     Sample Usage:
    ///     <para />
    ///     ActionOnEachChainConnectPoint(connectPoint =>
    ///     {
    ///     connectPoint.GenerateSeal();
    ///     });
    /// </summary>
    ...
    protected virtual LegacyChainStructure GetNewStructure(ChainConnectPoint parentConnectPoint,
    ...
    protected virtual Bridge GetBridgeOfDirection(Bridge[] bridges, byte direction, LegacyChainStructure structure) {
    ...
    protected virtual (int newStructureConnectPointX, int newStructureConnectPointY) SetProspectiveStructurePosition(
        LegacyChainStructure structure, ChainConnectPoint connectPoint, Bridge bridge) {
        int deltaX, deltaY;
    ...
    /// <summary>
    ///     Called for each LegacyChainStructure right after it gets generated.
    /// </summary>
    protected virtual void OnStructureGenerate(LegacyChainStructure structure) {
    }
}
