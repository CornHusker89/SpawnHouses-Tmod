...
namespace SpawnHouses.StructureHelper;
...
public static class Generator {
    ...
    /// </summary>
    /// <param name="path">
    ///     The path to your structure file within your mod - this should not include your mod's folder, only
    ///     the path beyond it.
    /// </param>
    ...
    /// <param name="fullPath">
    ///     Indicates if you want to use a fully qualified path to get the structure file instead of one
    ///     from your mod - generally should only be used for debugging.
    /// </param>
    /// <param name="ignoreNull">
    ///     If the structure should repsect the normal behavior of null tiles or not. This should never be
    ///     true if you're using the mod as a dll reference.
    /// </param>
    /// <param name="flags">Allows you to pass flags for special generation behavior. See <see cref="GenFlags" /> </param>
    /// <param name="tileChances">
    ///     Dict of the tiles that don't have a 100% chance to spawn. value represents 0-100 percent
    ///     chance to spawn.
    /// </param>
    ...
    public static bool GenerateStructure(string path, Point16 pos, Mod mod, bool fullPath = false,
        ...
        if (tag.ContainsKey("Structures"))
            throw new Exception(
                $"Attempted to generate a multistructure '{path}' as a structure. Use GenerateMultistructureRandom or GenerateMultistructureSpecific instead.");

        return Generate(tag, pos, ignoreNull, flags, tileChances);
    }
    ...
    /// </summary>
    /// <param name="path">
    ///     The path to your multistructure file within your mod - this should not include your mod's folder,
    ///     only the path beyond it.
    /// </param>
    /// <param name="pos">The position in the world in which you want your structure to generate, in tile coordinates.</param>
    /// <param name="mod">The instance of your mod to grab the file from.</param>
    /// <param name="fullPath">
    ...
    /// <param name="ignoreNull">
    ///     If the structure should repsect the normal behavior of null tiles or not. This should never be
    ///     true if you're using the mod as a dll refference.
    /// </param>
    /// <param name="flags">Allows you to pass flags for special generation behavior. See <see cref="GenFlags" /> </param>
    /// <returns>If the structure generated successfully or not</returns>
    ...
    /// <summary>
    ///     This method generates a structure you select from a multistructure file within your mod. Useful if you want to do
    ///     your own weighted randomization or want additional logic based on dimensions gotten from
    ...
    /// <param name="mod">The instance of your mod to grab the file from.</param>
    /// <param name="index">
    ///     The index of the structure you want to generate out of the multistructure file, structure indicies
    ///     are 0-based and match the order they were saved in.
    /// </param>
    /// <param name="fullPath">
    ///     Indicates if you want to use a fully qualified path to get the structure file instead of one
    ///     from your mod - generally should only be used for debugging.
    ...
    /// <param name="mod">The instance of your mod to grab the file from.</param>
    /// <param name="index">
    ///     The index of the structure you want to get the dimensions of out of the multistructure file,
    ///     structure indicies are 0-based and match the order they were saved in.
    /// </param>
    /// <param name="dims">The Point16 variable which you want to be set to the dimensions of the structure.</param>
    /// <param name="fullPath">
    ///     Indicates if you want to use a fully qualified path to get the structure file instead of one
    ...
    /// <summary>
    ///     Checks if a structure file is a multistructure or not. Can be used to easily add support for parameterizing
    ///     strucutres or multistructures in your mod.
    /// </summary>
    ...
    /// <param name="tileChances">
    ///     A Dict of the tiles that don't have a 100% chance to spawn. value represents 0-100 percent
    ///     chance
    /// </param>
    ...
    public static unsafe bool Generate(TagCompound tag, Point16 pos, bool ignoreNull = false,
        GenFlags flags = GenFlags.None, Dictionary<ushort, byte> tileChances = null) {
        ...
        for (var y = 0; y <= height; y++) {
                ...
                if (d.TEType != "") //place and load a tile entity
                {
                    if (d.TEType == "StructureHelper ChestEntity" && !ignoreNull) {
                        ...
                        if (typ != 0) {
                            ...
                            if (d.TEData != null &&
                                typ != 2) // We specifically exclude logic sensors (type 2) because these store their absolute pos
                                TileEntity.ByPosition[new Point16(pos.X + x, pos.Y + y)].LoadData(d.TEData);
                        }
                    }
                }
                else if ((type == TileID.Containers || TileID.Sets.BasicChest[tile.TileType]) && d.frameX % 36 == 0 &&
                         d.frameY % 36 == 0) //generate an empty chest if there is no chest data
                {
            ...
            if (!isNullWall || ignoreNull)
                ...
                tile.WallType =
                    oldWall; //revert to old wall if its a null wall and not a null tile, else would be overridden by the WallWireData struct
        }
