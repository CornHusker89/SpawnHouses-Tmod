...
public sealed class MainBasement_Room2 : CustomChainStructure
{
    // constants
    ...
    public MainBasement_Room2(sbyte cost, ushort weight, Bridge[] childBridgeType, byte status = StructureStatus.NotGenerated, 
        ushort x = 1000, ushort y = 1000) :
        base(_filePath, _structureXSize, _structureYSize, CopyFloors(_floors),
            CopyChainConnectPoints(_connectPoints), childBridgeType, status, x, y, cost, weight)
    {
        ID = StructureID.MainHouseBasement_Room2;
    ...
    {
        return new MainBasement_Room2(Cost, Weight, ChildBridgeTypes, Status, X, Y);
    }
}