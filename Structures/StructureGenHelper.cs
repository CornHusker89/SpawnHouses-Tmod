...
public static class StructureGenHelper
{
    ...
    /// <returns></returns>
    /// <exception cref="Exception"></exception>
    public static (bool found, int distance, int yCoord) SurfaceRaycastFromInsideTile(int x, int y, int maxCastDistance = 40)
    {
        bool inSolid = true;
        for (int i = 0; i < maxCastDistance; i++)
            if (Terraria.WorldGen.SolidTile(x, y + i))
    ...
    public static void Blend(Point start, ushort blendDistance, ushort topTileID, ushort fillTileID = 0, bool canUsePartialTiles = true,
        bool removeWalls = true, ushort maxHeight = 38, bool blendLeftSide = true)
    {
        ...
        for (int dX = blendDistance; dX >= 0; dX--)
        {
            ...
            ushort dYDown = 1;
            ushort fillCount = 0;
            while (((!Terraria.WorldGen.SolidTile(startX - dX, topTileY + dYDown) && dYDown <= lastTileVerticalFillLen * 1.3 + 2) || dYDown <= lastTileVerticalFillLen * 0.6) && fillCount < 25)
            {
        ...
        WorldUtils.Gen(new Point(frameCenterX, frameCenterY), new Shapes.Circle(radius: blendDistance * 6), new Actions.SetFrames());
    }

    public static void Blend(ConnectPoint start, ushort blendDistance, ushort topTileID, ushort fillTileID = 0, bool canUsePartialTiles = true,
        bool removeWalls = true, ushort maxHeight = 38, bool blendLeftSide = true)
    {
        Blend(new Point(start.X, start.Y), blendDistance, topTileID, fillTileID, canUsePartialTiles, removeWalls, maxHeight, blendLeftSide);
    }
}