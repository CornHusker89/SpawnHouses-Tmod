...
public abstract class AdvGeneratable<TSelf, TParams, TGenerator> : IAdvGeneratable<TSelf, TParams, TGenerator>
    ...
    protected void SetGenerator() {
        ...
        if (!StructureManager.AdvInstanceGenerators.TryGetValue(instanceType, out var generators)) throw new Exception($"instance type {GetType().FullName} generators not found");
        var typedGenerators = generators.Where(g => !g.Depreciated).ToArray();
