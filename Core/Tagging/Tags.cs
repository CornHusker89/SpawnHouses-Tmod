...
public static class Tags {
    ...
    /// if the structure has alternate structures that have similar tilemaps but are larger or "improved" in some other way. set automatically for loaded
    /// <see cref="FileStructureTemplate" />
    /// s
    public static Tag<StructureRoot[]> Structure_Upgradable = new();

    /// if the structure can only be generated through an upgrade; cannot be made directly as a new structure
    public static Tag Structure_PlacedOnlyByUpgrade = new();
    ...
    /// if the structure uses Magic Storage in storage areas
