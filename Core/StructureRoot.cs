...
public abstract class StructureRoot : IGeneratable, IStructureTags, IDebugDraw {
    ...
    /// <summary>
    ///     if another structure can be considered an "upgrade" to this structure
    /// </summary>
    /// <returns></returns>
    public bool IsUpgrade(StructureRoot candidate) {
        if (!TagsCurrent.GetValueSafe(Tags.Structure_HasHousing, out int thisHousing)) thisHousing = 0;
        if (!candidate.TagsCurrent.GetValueSafe(Tags.Structure_HasHousing, out int candidateHousing)) candidateHousing = 0;
        if (!TagsCurrent.GetValueSafe(Tags.Structure_HasStorage, out int thisStorage)) thisStorage = 0;
        if (!candidate.TagsCurrent.GetValueSafe(Tags.Structure_HasStorage, out int candidateStorage)) candidateStorage = 0;
        bool thisMs = TagsCurrent.HasTag(Tags.Structure_UsesMS);
        bool candidateMs = candidate.TagsCurrent.HasTag(Tags.Structure_UsesMS);

        // housing cap improvement
        if (candidateHousing > thisHousing)
            return true;

        // adds MS without significant storage compromise
        if (candidateHousing == thisHousing && !thisMs && candidateMs && candidateStorage >= thisStorage * 0.7)
            return true;

        // adds storage without housing change
        if (candidateHousing == thisHousing && (candidateMs == thisMs || candidateMs) && candidateStorage > thisStorage)
            return true;
